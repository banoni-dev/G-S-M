...
export function PropDivider() {
  return <div className="w-full h-[0.015rem] bg-LightGrayishBlue/50"></div>;
}

type PlanProps = (typeof plans)[number];

export function Plan({ name, active, price, features }: PlanProps) {
  let planClass = `flex flex-col justify-start items-center rounded-[0.65rem] px-[1.9rem] font-bold  ${
    active
      ? "h-[31.25rem] w-[21.875rem] bg-customGradientBottom py-[3.4rem] text-white"
      : "h-[28.4rem] w-[21.875rem] py-[1.9rem] bg-white text-"
  }`;
  ...
  return (
    <section className={planClass}>
      ...
      <h1
        className={`flex items-center gap-[0.7rem] text-xl -tracking-[0.03em] mt-[1.45rem] mb-[2.1rem] ${
      ...
      <button
        className={`h-[2.75rem] w-full text-xs tracking-[.17em] rounded-md mt-[1.95rem] uppercase border-[0.1rem] ${
